...
export class AuditService {
  ...
  private async progressiveGoto(page: Page, url: string): Promise<HTTPResponse | null> {
    const considerReady = async () => {
      try {
        return await page.evaluate(() => {
          ...
          // Try to detect common SPA roots to avoid bailing early
    ...
    // Simple SPA guess (extra time if HTML-like)
    const isSPA = true; // JS-heavy sites benefit from a larger budget by default
    const totalBudgetMs = isSPA ? 180_000 : 120_000;
    ...
    const strategies: Array<{ name: string; opts: Parameters<Page['goto']>[1] }> = [
      ...
      { name: 'networkidle0',    opts: { waitUntil: 'networkidle0',     timeout: 45_000 } },
    ...
    for (let i = 0; i < strategies.length; i++) {
      ...
      try {
        ...
        try { await page.waitForSelector('body', { timeout: 5_000 }); } catch { /* ignore */ }

        const settleTimeMs = isSPA ? 15_000 : 8_000;
        const settleUntil = Date.now() + settleTimeMs;
        let readyChecks = 0;
        const maxChecks = isSPA ? 40 : 20;
        ...
        while (Date.now() < settleUntil && readyChecks < maxChecks) {
          if (await considerReady()) {
            ...
            if (isSPA) await this.sleep(2_000); // small grace for final paint
            return resp;
          }
          await this.sleep(isSPA ? 500 : 400);
          readyChecks++;
        }

        // If we have a response but not fully ready, keep trying other strategies;
        // otherwise, return what we have on the last strategy.
        if (resp && i === strategies.length - 1) return resp;
      } catch (e) {
    ...
    // Ultimate salvage: if *any* content exists, proceed
    try {
      const hasAnyContent = await page.evaluate(() => !!document.body && document.body.innerHTML.length > 0);
      if (hasAnyContent) {
        console.warn('[audit] found page content, proceeding with salvage audit');
