...
const KNOBS = {
    // Browser / job budgets
    jobBudgetMs: 360_000,   // was 180_000; give slow SPAs breathing room
    navBudgetMs: 100_000,   // was 120_000; spend less before post-nav work
    preflightMs: 6_000,     // was 8_000; shave some front-loaded time

    // ProgressiveGoto per-attempt timeouts (kept sensible, slightly leaner)
    gotoAttempts: [
        { name: 'domcontentloaded', waitUntil: 'domcontentloaded' as const, timeout: 35_000 },
        { name: 'load', waitUntil: 'load' as const, timeout: 30_000 },
        // networkidle* is too brittle on many SPAs; skip it by default
        { name: 'basic', waitUntil: undefined, timeout: 20_000 },
    ],

    // “ready enough” settle windows (leaner to avoid long stalls)
    settleMsSPA: 8_000,        // was 12_000
    settleMsRegular: 4_000,    // was 6_000
    settlePollMsSPA: 450,      // was 500
    settlePollMsRegular: 300,  // was 350

    // Lazy-load nudge
    doGentleScroll: true,
    scrollSteps: 3,            // was 4
    scrollPauseMs: 400,        // was 500
...
export class AuditService {
    ...
    private async processAudit(request: AuditRequest): Promise<void> {
        ...
        try {
            ...
            // Start watchdog LATE, using only the remaining time
            const remaining = Math.max(30_000, KNOBS.jobBudgetMs - (Date.now() - jobStart));
            const jobAbort = new Promise<never>((_, rej) =>
                setTimeout(() => rej(new Error('Job watchdog timeout')), remaining)
            );
