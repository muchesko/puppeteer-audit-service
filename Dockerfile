...
COPY tsconfig.json ./
COPY src ./src
# add any other build-time files here (e.g., ./config if needed)
RUN npm run build
...
# Install Chrome Stable + full set of headless runtime libs + curl for healthcheck
RUN apt-get update && apt-get install -y --no-install-recommends \
      wget gnupg ca-certificates curl \
      fonts-liberation ttf-freefont \
...
# Puppeteer should NOT download Chromium; point to system Chrome
ENV PUPPETEER_SKIP_CHROMIUM_DOWNLOAD=true
ENV PUPPETEER_EXECUTABLE_PATH=/usr/bin/google-chrome-stable
# Memory tuning for small instances
...
# If you need runtime assets/config:
# COPY config ./config
